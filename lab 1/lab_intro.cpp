...
PNG grayscale(PNG image) {
  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      double adjusted_blue = 0.114 * pixel->r;
  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      if (dist >= 200) {
        ...
        brightness = 1 - (dist * 0.005);
      }
      pixel->r = pixel->r * brightness;
      pixel->g = pixel->g * brightness;
      pixel->b = pixel->b * brightness; 
    }
  }
  return image;
}
...
PNG ubcify(PNG image) {
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      if(colordist(*pixel, yellow) > colordist(*pixel, blue)) {
        *pixel = blue;
      } else {
        *pixel = yellow;
      }
...
PNG watermark(PNG firstImage, PNG secondImage) {
  // firstImage.resize(min(firstImage.width(), secondImage.width())
  //   ,min(firstImage.height(), secondImage.height()));
  // secondImage.resize(min(firstImage.width(), secondImage.width())
  //   ,min(firstImage.height(), secondImage.height()));
