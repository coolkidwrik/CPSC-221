...
public:
     ...
     *  on a pruned subtree. (we only expect that trees would be pruned once.)
     *
     *  This is a recursive helper function for prune().
     */
    void pruneh(Node* node, const int tol);
    ...
    /*
     * helper for pruneSize
     * 
     */
    int traverse(Node* n, const int tol) const;
...
private:
     ...
     * sets all pointers in node to nullptr
     * deletes node
     */
