...
void AVLTree<K, V>::rebalance(Node*& subtree)
{
    ...
    //     return;
    // }
    int balance_factor = height(subtree->left) - height(subtree->right);

//    if(abs(height(subtree->right) - height(subtree->left)) <= 1) {
//         updateHeight(subtree);
//     } if (height(subtree->right) - height(subtree->left) < -1) { // cases 1-2 (left heavy)
//         if (height(subtree->left->right) <= height(subtree->left->left)) { // right rotation
//             rotateRight(subtree);
//         } else { // left-right rotation
//             rotateLeftRight(subtree);
//         }
//     } else if (height(subtree->right) - height(subtree->left) > 1) {
//         if (height(subtree->right->right) >= height(subtree->right->left)) { // right rotation
//             rotateLeft(subtree);
...
//     }
    ...
    //     updateHeight(subtree);
    // } if (balance_factor > 1) { // cases 1-2 (left heavy)
    //     if (height(subtree->left->left) - height(subtree->left->right) >= 1) { // right rotation
    //         rotateRight(subtree);
    //     } else { // left-right rotation
    //         rotateLeftRight(subtree);
    //     }
    // } else if (balance_factor < -1) {
    //     if (height(subtree->right->left) - height(subtree->right->right) <= -1) { // right rotation
    //         rotateLeft(subtree);
    //     } else { // right-left rotation
    //         rotateRightLeft(subtree);
    //     }
    ...
    if (abs(balance_factor) <= 1) {
        updateHeight(subtree);
    } else if (balance_factor > 1) { // cases 1-2 (left heavy)
        if (height(subtree->left->left) - height(subtree->left->right) >= 1) { // right rotation
            rotateRight(subtree);
        } else { // left-right rotation
            rotateLeftRight(subtree);
        }
    } else if (balance_factor < -1) { // cases 3-4 (left heavy)
        if (height(subtree->right->left) - height(subtree->right->right) <= -1) { // left rotation
            //rotateLeft(subtree);
            rotateLeft(subtree);
        } else { // right-left rotation
            rotateRightLeft(subtree);
